...
from .project import building_geoloc,rtaa_sun_data
...
def intersect(shapes):
        return BRepAlgoAPI_Common(shapes[0],shapes[1]).Shape()



@profile
def compute_direct_mask_on_face(sun_dir,mybuilding,myface,theface_norm,pool):
    ...
    for s in lsolid:
        args.append((s,theface))
    lface2 = pool.map(intersect, args)
    print(lface2)
...
def compute_direct_mask_on_face_test(sun_dir,mybuilding,myface,theface_norm):
    """Compute the shadow geometry generated by my building on myface 
    trough solid clipping method for one solar vector sun_dir.
    ...
    # defining a very small face with nearly zero area
    # to return in case of no shadow
    gpp=GProp_GProps()
    brepgprop.SurfaceProperties(theface,gpp)
    mass_center=gpp.CentreOfMass()
    #print(' face     ',gpp.Mass())

    scale=gp_Trsf()
    scale.SetScale(mass_center,1e-5)
    builder = BRepBuilderAPI_Transform(scale)
    builder.Perform(theface)
    miniface=builder.ModifiedShape(theface)
    
    brepgprop.SurfaceProperties(miniface,gpp)
    
    #print('mini face ',gpp.Mass())
    ...
    lface2=[]
    for s in lsolid:
        lface2.append(intersect((s,theface)))
    #print(lface2)
    ...
    #print('shadow ',shadowface)
    
    return shadowface


#@profile
def compute_direct_mask_on_face_parallel(sun_dir,mybuilding,myface,theface_norm):
    """Compute the shadow geometry generated by my building on myface 
    trough solid clipping method for one solar vector sun_dir.
    ...
    # extrusion of selected faces
    extrusions = TopTools_ListOfShape()
    face_los   = TopTools_ListOfShape()
    all_los    = TopTools_ListOfShape()
    void_los   = TopTools_ListOfShape()
    
    temp=[]
    for ft in lfaces :
        extru = BRepPrimAPI_MakePrism(ft,ext_vec,False,True).Shape()
        extrusions.Append(extru)
        all_los.Append(extru)
        
        temp.append(extru)
        
    #print(extrusions.Size())
    # void face with zero area
    if(extrusions.Size()==0):
        return TopoDS_Face() 
    
    all_los.Append(theface)
    face_los.Append(theface)
    #los_totake = TopTools_ListOfShape()
    #los_totake.Append(theface)
    
    #lsolid.Append(theface)
    # fusion of extrusions
    cb=BOPAlgo_CellsBuilder()
    cb.SetNonDestructive(True)
    cb.SetArguments(all_los)
    ...
    """
    cb.RemoveAllFromResult()
    cb.AddToResult(face_los,extrusions,2)
    """
    cb.RemoveAllFromResult()
    cb.AddToResult(face_los,void_los,2)
    cb.RemoveFromResult(face_los,extrusions)
    cb.RemoveInternalBoundaries()
    
    shadowface=cb.Shape()
    """
    if(extrusions.Size()>10):
        display, start_display, add_menu, add_function_to_menu = init_display('pyqt5')
        
        display.DisplayShape(theface,color='BLUE',transparency=0.1)
        display.DisplayShape(shadowface,color='RED',transparency=0.5)
        #display.DisplayShape(shadowface1,color='BLACK',transparency=0.5)
        #[display.DisplayShape(t,color='GREEN',transparency=0.99)for t in temp ]
        display.FitAll()
    ...
    """
    # intersection with face of interest
    lsolid = TopTools_ListOfShape()
    lsolid.Append(fused_shadow)
    lsolid.Append(theface)
    
    cb=BOPAlgo_CellsBuilder()
    cb.SetNonDestructive(True)
    cb.SetArguments(lsolid)
    cb.SetFuzzyValue(precision)
    cb.SetRunParallel(True)
    cb.Perform()
    cb.RemoveAllFromResult()
    cb.AddToResult(lsolid,los_avoid,2)
    cb.RemoveInternalBoundaries()
    shadowface=cb.Shape()
    """    
...
class direct_mask_on_face:
    ...
    def compute_mask(self,exposed_building):
        ...
        for j,sun_dir in enumerate(self._lsun_dir):
            #btemp=BRepBuilderAPI_Copy(exposed_building).Shape()
            #ftemp=BRepBuilderAPI_Copy(self._face).Shape()
            args.append([sun_dir,exposed_building,self._face,face_norm])
            #mask_face=compute_direct_mask_on_face(sun_dir,exposed_building,self._face,face_norm,P)
            
            
            #print('     sun dir ',j,'/',len(self._lsun_dir)-1)#,end="",flush=True)
            #print('output ',mask_face)
            
            #self._mask_faces.append(mask_face)
            #print(len(self._mask_faces))
        with  multiprocessing.Pool() as P :
            wrapped_args=tqdm.tqdm(args,total=len(args))
            self._mask_faces=P.starmap(compute_direct_mask_on_face_test,wrapped_args)
    ...
    def display(self,exposed_building):
        ...
        lvec=[gp_Vec(d) for d in self._lsun_dir if  face_norm.Dot(gp_Vec(d))<-1e-5]
...
class rtaa_on_faces:
    ...
    def display(self):
        ...
        gpp=GProp_GProps()
        to_show=self._lfaces
        if self._adjust:
            toshow=self._ladjusted
...
class rtaa_solar:
    ...
    def set_geometry(self, face, building_lshapes):
        self._face=face
        srf = BRep_Tool().Surface(face)
        self._plane = Geom_Plane.DownCast(srf)
        self._face_norm = self._plane.Axis().Direction()
        if(self._face.Orientation()==1):
            self._face_norm.Reverse()
        
        #print(building_lshapes)
        solid = fuse_listOfShape(building_lshapes)
        #print(solid)
        if (solid is not None ) :
            shu=ShapeUpgrade_UnifySameDomain(solid)
            shu.Build()
            solid=shu.Shape() 
        self._building=solid
    ...
    def display(self):
        ...
        display.DisplayShape(self._face,color='RED',transparency=0.5)
        
        gpp=GProp_GProps()
        norm=gp_Vec(self._face_norm)
        brepgprop.SurfaceProperties(self._face,gpp)
        mc=gpp.CentreOfMass()
        display.DisplayVector(norm,mc)
    ...
    def run(self):
        norm_tn=self._geoloc.face_normal_to_tn(self._face)
        sun_to_earth_project,irradiance=self._sun_data.sun_data(norm_tn)
           
        self._rtaa=rtaa_on_faces([self._face],
                                  self._plane,
                                  self._building,
        ...
        self._rtaa.compute_masks()
        self._rtaa.compute_cm(irradiance)
        
    
class rtaa_solar_ifc:
