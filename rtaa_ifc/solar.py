...
def compute_direct_mask_on_face(sun_dir,mybuilding,myface,theface_norm):
    ...
    # It is necessary to use the funtion with starmap since it
    # is not possible to pickle a null object
    gpp=GProp_GProps()
...
class direct_mask_on_face:
    ...
    def compute_mask(self,exposed_building):
        ...
        with  multiprocessing.Pool() as P :
            ...
            self._mask_faces=P.starmap(compute_direct_mask_on_face,wrapped_args)
...
class rtaa_solar_ifc:
    ...
    def run(self,cut=True,adjust=True):
        ...
        for id,facelist in self._solar_faces.items():
            ...
            to_tn=self._geoloc.face_normal_to_tn(facelist[0])
            sector = self._geoloc.orientation_sector(to_tn)
            sun_to_earth_project,irradiance=self._sun_data.sun_data(sector)
